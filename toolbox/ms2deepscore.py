...
class SpectrumBinner:
    ...
    def from_json(cls, json_str: str):
        ...
        if binner_dict["peak_to_position"] is not None:
            ...
            spectrum_binner.known_bins = [0 for _ in range(binner_dict["number_of_bins"])]
    ...
    def process_transfrom_wo_fit(self,spectrum):
        weights = spectrum.peaks.intensities ** self.peak_scaling
        bins = np.linspace(self.mz_min, self.mz_max + self.mz_min, self.number_of_bins)
        mapped_indices = []
        mapped_values = weights
        for mz in spectrum.peaks.mz:
            index = np.argmin(np.abs(bins - mz))
            mapped_indices.append(index)
        spectrum_bin = BinnedSpectrum(binned_peaks=create_peak_dict(list(zip(mapped_indices,mapped_values))),metadata=spectrum._metadata)
        return spectrum_bin

    
    def transfrom_wo_fit(self, input_spectrums: List[SpectrumType],progress_bar=True)-> List[BinnedSpectrumType]:
        spectrums_binned = []
        pbar = tqdm(total=len(input_spectrums),desc="Create BinnedSpectrum instances",disable=(not progress_bar))
        with Pool(processes=16) as pool:
            for spectrum_bin in pool.imap_unordered(self.process_transfrom_wo_fit,
                                        [spectrum for spectrum in input_spectrums]):
                spectrums_binned.append(spectrum_bin)
                pbar.update(1)
    ...
    def transform(self, input_spectrums: List[SpectrumType],
        ...
        for i, peak_list in enumerate(tqdm(peak_lists,
            ...
            spectrum = BinnedSpectrum(binned_peaks=create_peak_dict(peak_list),
                                      metadata=input_spectrums[i]._metadata)
