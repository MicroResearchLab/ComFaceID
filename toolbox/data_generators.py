...
class DataGeneratorBase(Sequence):
    ...
    def _collect_and_validate_inchikeys(self):
        ...
        """
        self.spectrum_inchikeys = np.array([s.get("inchikey")[:inchikey_subindex] for s in self.binned_spectrums])
    ...
    def _transform_to_inchikey14(reference_scores_df: pd.DataFrame):
        ...
        reference_scores_df.index = [x[:inchikey_subindex] for x in reference_scores_df.index]
        reference_scores_df.columns = [x[:inchikey_subindex] for x in reference_scores_df.columns]
        if len(reference_scores_df.index) != len(list(set(reference_scores_df.index))):
            print("⚠️ Potential inchikey subset conflicts monitored, please increase <inchikey_subindex>={} to solve".format(inchikey_subindex))
    ...
    def _data_augmentation(self, spectrum_binned):
        ...
        """
        if self.settings["need_energy"]:
            idx = np.array([int(x)+2 for x in spectrum_binned.keys()])
        else:
            idx = np.array([int(x)+1 for x in spectrum_binned.keys()])
        ...
        # np.insert(idx,0,[0])
        # np.insert(values,0,[spectrum._metadata["pepmass"][0]])
        return idx, values
    ...
    def __data_generation(self, spectrum_pairs: Iterator[SpectrumPair]):
        ...
        # Initialization
        if self.settings["need_energy"]:
            X = [np.zeros((self.settings["batch_size"], self.dim+2)) for i in range(2)]
        else:
            X = [np.zeros((self.settings["batch_size"], self.dim+1)) for i in range(2)]
        ...
        for i_pair, pair in enumerate(spectrum_pairs):
            for i_spectrum, spectrum in enumerate(pair):
                ...
                mass = spectrum._metadata["pepmass"][0]
                energy = spectrum._metadata["collision_energy"]
                X[i_spectrum][i_pair, idx] = values
                X[i_spectrum][i_pair, 0] = mass
                if self.settings["need_energy"]:
                    X[i_spectrum][i_pair, 1] = energy
            score = self.reference_scores_df[pair[0].get("inchikey")[:inchikey_subindex]][pair[1].get("inchikey")[:inchikey_subindex]]
            y[i_pair] = score 
        return X, y
...
class DataGeneratorAllSpectrums(DataGeneratorBase):
    ...
    def _spectrum_pair_generator(self, batch_index: int) -> Iterator[SpectrumPair]:
        ...
        for index in indexes:
            ...
            inchikey1 = spectrum1.get("inchikey")[:inchikey_subindex]
    ...
    def _exclude_not_selected_inchikeys(self, reference_scores_df: pd.DataFrame) -> pd.DataFrame:
        ...
        inchikeys_in_selection = {s.get("inchikey")[:inchikey_subindex] for s in self.binned_spectrums}
