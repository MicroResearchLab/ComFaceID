...
preinfer_embeddings = "embed_database/database_embedding.pkl"
...
for file in os.listdir(SourceDatabasePath):
    ...
    if (file.lower().endswith(".mgf") or file.lower().endswith(".mzxml")):
        ...
        tmp_file = str(TmpPath / (origin.split(".")[0] + "__tmp__.mgf"))
...
def process(input):
    i, embeddings, reference_vector, sourceSpec = input
    res_list = []
    mass = float(sourceSpec.get("pepmass")[0])
    for embedding in embeddings:
        if filterByMass > 0 and abs(float(embedding["mass"]) - mass) >= filterByMass:
            continue
        if embedding["mid"][0].isdigit():
            embedding["mid"] = "MID" + embedding["mid"]
        if filterByFormula and not checkFormulaSim(embedding["formula"], chemparse.parse_formula(sourceSpec.get("formula"))):
            continue
        sim = cosine_similarity(
            reference_vector, np.array(embedding["embedding"], dtype=float))
        res = {
                # **target_info,
                "mid": embedding["mid"],
                # "mass": embedding["mass"],
...
if params.predict_fpr:
    ...
    file = open(preinfer_embeddings, "rb")
    preinfer_embedding_list = pickle.load(file)
    list = preinfer_embedding_list
sim_res_top_map = []
class_res = []
count = 0
...
for sourceSpec in sourceSpectrums[:]:
    ...
    embedding_vector = torch.tensor(
            reference_vector, dtype=torch.float32).cuda()
    ...
    if params.predict_fpr:
        ...
        with Pool(processes=num_workers) as pool:
            for res in pool.imap_unordered(processFpr,
                                           [[i, embeddings, pred_fpr, sourceSpec, mass]
                                            for i, embeddings in enumerate(chunks(list, num_workers*20))]):
                if res is not None:
                    local_list += res
                    pbar.update(len(res))
        pbar.close()
        ...
        pbar = bar(total=len(list),mininterval=1)
        with Pool(processes=num_workers) as pool:
            for res in pool.imap_unordered(process,
                                           [[i, embeddings, reference_vector, sourceSpec]
                                            for i, embeddings in enumerate(chunks(list, num_workers))]):
                if res is not None:
                    local_list += res
                    pbar.update(len(res))
        pbar.close()
    ...
    sim_tops_ids = argsort(local_list, key="sim", reverse=True)
    ...
    if not params.predict_fpr:
        res_list_extra = []
        for item in res_list:
            target_info = {}
            try:
                target_info = df.search_one(item["mid"])
                del target_info["id"]
            except Exception as e:
                pass
            del item["mid"]
            # del item["targetSpec"]["bit_fp"]
            # del item["targetSpec"]["pubchem_catav"]
            # del item["targetSpec"]["MACCS"]
            del target_info["sirius_pubchem_fp"]
            del target_info["sirius_maccs_fp"]
            item = {
                **item,
                **target_info
            }
            res_list_extra.append(
                item
            )
        res_list = res_list_extra
    class_res +={"precursor":mass,"file":sourceFiles[count][1],"index":str(sourceFiles[count][0]),"class_pred":c_pred,"superclass_pred":sc_pred}
