...
import Library.ming_psm_library as psm_library
import Library.ming_numerical_utilities as numerical_utilities
import Library.ming_sptxt_library as sptxt_library
...
class Spectrum:
    ...
    def filter_noise_peaks(self, min_snr):
        average_noise_level = numerical_utilities.calculate_noise_level_in_peaks(self.peaks)
        new_peaks = []
        for peak in self.peaks:
    ...
    def generated_spectrum_vector(self, peptide=None, attenuation_ratio=0.0, tolerance=0.5, bin_size=1):
        ...
        if peptide != None:
            ...
            theoretical_peaks = psm_library.create_theoretical_peak_map(self.peptide, ["b", "y", "b-H2O", "b-NH3", "y-H2O", "y-NH3", "a"], charge_set=charge_set)
            annotated_peaks, unannotated_peaks = psm_library.extract_annotated_peaks(theoretical_peaks, self.peaks, tolerance)
        ...
        #Doing
        peak_vector = numerical_utilities.vectorize_peaks(self.peaks, max_mass, bin_size)
...
class PeptideLibrarySpectrum(Spectrum):
    ...
    def get_annotated_peak_count(self, tolerance):
        annotated_peak_count = psm_library.calculated_number_annotated_peaks(self.peaks, self.charge, self.peptide, tolerance)
    ...
    def get_sptxt_string(self):
        output_lines = []
        annotations_line = "Name: " + sptxt_library.transform_peptide_to_msp_library_string(self.peptide) + "/" + str(self.charge)
        output_lines.append(annotations_line)

        mod_string = sptxt_library.transform_peptide_to_msp_mods(self.peptide)
    ...
    def get_tsv_line(self, output_mgf_filename=""):
        length_of_peptide = len(psm_library.strip_sequence(self.peptide))
...
def calculate_unique_ions_annotated(peaks, max_charge, peptide, tolerance):
    ...
    theoretical_peaks = psm_library.create_theoretical_peak_map(peptide, ["b", "b-iso", "y", "y-iso", "b-H2O", "b-NH3", "y-H2O", "y-NH3", "a"], charge_set=charge_set)
...
def map_ions_to_peak(peaks, max_charge, tolerance, peptide, ions_to_consider=["b", "y", "b-H2O", "b-NH3", "y-H2O", "y-NH3", "a"]):
    charge_set = range(1, max_charge + 1)
    theoretical_peaks = psm_library.create_theoretical_peak_map(peptide, ions_to_consider, charge_set=charge_set)
...
def determine_b_y_breaks_total(peaks, max_charge, tolerance, peptide, SNR=2.0):
    ...
    peptide_length = len(psm_library.strip_sequence(peptide))
...
def attenuate_unannotated_peaks(peaks, max_charge, tolerance, peptide, attenuation_ratio=0):
    ...
    theoretical_peaks = psm_library.create_theoretical_peak_map(peptide, ["b", "y", "b-H2O", "b-NH3", "y-H2O", "y-NH3", "a"], charge_set=charge_set)
    annotated_peaks, unannotated_peaks = psm_library.extract_annotated_peaks(theoretical_peaks, peaks, tolerance)
    new_peaks = annotated_peaks
    if attenuation_ratio > 0:
        for unannotated_peak in unannotated_peaks:
            unannotated_peak[1] *= attenuation_ratio
